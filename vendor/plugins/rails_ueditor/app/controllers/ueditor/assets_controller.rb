...
class Ueditor::AssetsController < ApplicationController
  ...
  def create
    ...
    if !RailsUeditor.image_resize_to_fit.nil? && @dir == 'image'
      Ueditor::AssetUploader.send(:include, CarrierWave::MiniMagick)
      Ueditor::AssetUploader.send(:process, :resize_to_fit => RailsUeditor.resize_to_fit)      
    end
    unless @imgFile.nil?
      if Ueditor::AssetUploader.save_upload_info? # save upload info into database
        begin
          @asset = "Ueditor::#{@dir.camelize}".constantize.new(:asset => @imgFile)
          if @asset.save
        ...
        begin
          uploader = "Ueditor::#{@dir.camelize}Uploader".constantize.new
  ...
  def list
    @root_path = "#{Rails.public_path}/#{RailsUeditor.upload_store_dir}/"
    @root_url = "/#{RailsUeditor.upload_store_dir}/"
    @img_ext = Ueditor::AssetUploader::EXT_NAMES[:image]
    @dir = params[:dir].strip || ""
    unless Ueditor::AssetUploader::EXT_NAMES.keys.map(&:to_s).push("").include?(@dir)
      render :text => "Invalid Directory name."
      return
    end
    
    Dir.chdir(Rails.public_path)
    RailsUeditor.upload_store_dir.split('/').each do |dir|
